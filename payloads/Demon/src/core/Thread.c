...
#else
                        if ( NT_SUCCESS( ((INSTANCE *)Instance)->Win32.NtQueryVirtualMemory( NtCurrentProcess(), C_PTR( ThdCtx.Esp ), MemoryBasicInformation, &Memory1, sizeof( Memory1 ), NULL ) ) )
#endif
                        {
                            /* query memory info about rsp address */
...
HANDLE ThreadCreate(
    ...
    switch ( Method )
    {
        ...
        case THREAD_METHOD_CREATEREMOTETHREAD: {
            Thread = ((INSTANCE *)Instance)->Win32.CreateRemoteThread( Process, NULL, 0, Entry, Arg, 0, ThreadId );
            break;
        }
