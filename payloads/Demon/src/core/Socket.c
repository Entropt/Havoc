...
BOOL RecvAll( SOCKET Socket, PVOID Buffer, DWORD Length, PDWORD BytesRead )
{
    ...
    while ( tret < Length )
    {
        nret = ((INSTANCE *)Instance)->Win32.recv( Socket, Start, Length - tret, 0 );
...
BOOL InitWSA( VOID )
{
    ...
    /* Init Windows Socket. */
    if ( ((INSTANCE *)Instance)->WSAWasInitialised == FALSE )
    {
        PUTS( "Init Windows Socket..." )

        if ( ( Result = ((INSTANCE *)Instance)->Win32.WSAStartup( MAKEWORD( 2, 2 ), &WsData ) ) != 0 )
        {
        ...
        ((INSTANCE *)Instance)->WSAWasInitialised = TRUE;
    }
...
PSOCKET_DATA SocketNew( SOCKET WinSock, DWORD Type, BOOL UseIpv4, DWORD IPv4, PBYTE IPv6, DWORD LclPort, DWORD FwdAddr, DWORD FwdPort, DWORD ParentID )
{
    ...
    if ( ( Type != SOCKET_TYPE_NONE ) && ( Type != SOCKET_TYPE_CLIENT ) )
    {
        ...
        if ( UseIpv4 )
        {
            WinSock = ((INSTANCE *)Instance)->Win32.WSASocketA( AF_INET, SOCK_STREAM, IPPROTO_TCP, NULL, 0, NULL );
        ...
        {
            WinSock = ((INSTANCE *)Instance)->Win32.WSASocketA( AF_INET6, SOCK_STREAM, IPPROTO_TCP, NULL, 0, 0 );
            if ( WinSock == INVALID_SOCKET )
            {
                PRINTF( "WSASocketA Failed: %d\n", NtGetLastError() )
                goto CLEANUP;
        ...
        if ( Type == SOCKET_TYPE_REVERSE_PROXY )
        {
            if ( UseIpv4 )
            {
                ...
                if ( ((INSTANCE *)Instance)->Win32.connect( WinSock, ( struct sockaddr * ) &SockAddr, sizeof( SOCKADDR_IN ) ) == SOCKET_ERROR )
                {
            ...
            {
                /* connect to host:port */
                if ( ((INSTANCE *)Instance)->Win32.connect( WinSock, ( struct sockaddr * ) &SockAddr6, sizeof( SOCKADDR_IN6_LH ) ) == SOCKET_ERROR )
                {
                    PRINTF( "connect failed: %d\n", NtGetLastError() )
                    goto CLEANUP;
                }
            ...
            if ( ((INSTANCE *)Instance)->Win32.ioctlsocket( WinSock, FIONBIO, &IoBlock ) == SOCKET_ERROR )
            {
        ...
        else
        {
            ...
            /* bind the socket */
            if ( ((INSTANCE *)Instance)->Win32.bind( WinSock, ( struct sockaddr * ) &SockAddr, sizeof( SOCKADDR_IN ) ) == SOCKET_ERROR )
            {
            ...
            /* now listen... */
            if ( ((INSTANCE *)Instance)->Win32.listen( WinSock, 1 ) == SOCKET_ERROR )
            {
    ...
    Socket->Next         = ((INSTANCE *)Instance)->Sockets;

    ((INSTANCE *)Instance)->Sockets = Socket;
...
CLEANUP:
    if ( WinSock && WinSock != INVALID_SOCKET )
    {
        ...
        ((INSTANCE *)Instance)->Win32.closesocket( WinSock );
...
VOID SocketClients()
{
    ...
    for ( ;; )
    {
        ...
        {
            /* accept connection */
            WinSock = ((INSTANCE *)Instance)->Win32.accept( Socket->Socket, NULL, NULL );
            if ( WinSock != INVALID_SOCKET )
            {
                /* set socket to non blocking */
                if ( ((INSTANCE *)Instance)->Win32.ioctlsocket( WinSock, FIONBIO, &IoBlock ) != SOCKET_ERROR )
    ...
    for ( ;; )
    {
        ...
        if ( Socket->Type == SOCKET_TYPE_CLIENT || Socket->Type == SOCKET_TYPE_REVERSE_PROXY )
        {
            ...
            do
            {
                 ...
                 */
                if ( ((INSTANCE *)Instance)->Win32.ioctlsocket( Socket->Socket, FIONREAD, &PartialData.Length ) == SOCKET_ERROR )
                {
                    ...
                    Failed    = TRUE;
                    ErrorCode = ((INSTANCE *)Instance)->Win32.WSAGetLastError();
                }
...
VOID SocketFree( PSOCKET_DATA Socket )
{
    ...
    if ( Socket->Socket )
    {
        ((INSTANCE *)Instance)->Win32.closesocket( Socket->Socket );
        Socket->Socket = 0;
    }
...
VOID SocketCleanDead()
{
    ...
    for ( ;; )
    {
        ...
        if ( Socket->ShouldRemove )
        {
            ...
            if ( ! SkLast )
            {
                ((INSTANCE *)Instance)->Sockets = Socket->Next;
                SocketFree( Socket );
                Socket = ((INSTANCE *)Instance)->Sockets;
            }
            else
...
DWORD DnsQueryIPv4( LPSTR Domain )
{
    ...
    Ret = ((INSTANCE *)Instance)->Win32.getaddrinfo( Domain, NULL, &hints, &res );
    if ( Ret != 0 )
    {
    ...
    ((INSTANCE *)Instance)->Win32.freeaddrinfo( res );
