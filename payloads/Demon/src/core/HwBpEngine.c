...
NTSTATUS HwBpEngineInit(
    ...
    if ( ( ! HwBpEngine && ! HwBpHandler ) && ((INSTANCE *)Instance)->HwBpEngine ) {
        return STATUS_INVALID_PARAMETER;
    }

    if ( ((INSTANCE *)Instance)->HwBpEngine ) {

    }

    /* since we did not specify an engine let's use the global one */
    if ( ! HwBpEngine ) {
        HwBpEngine  = ((INSTANCE *)Instance)->HwBpEngine = MmHeapAlloc( sizeof( HWBP_ENGINE ) );
        HwBpHandler = &ExceptionHandler;
    }

    /* register Vectored exception handler */
    if ( ! ( HwBpEngine->Veh = ((INSTANCE *)Instance)->Win32.RtlAddVectoredExceptionHandler( TRUE, HwBpHandler ) ) ) {
...
NTSTATUS HwBpEngineSetBp(
    ...
    DWORD             Pid     = ((INSTANCE *)Instance)->Session.PID;
    CLIENT_ID         Client  = { 0 };
    CONTEXT           Context = { 0 };
    HANDLE            Thread  = NULL;
...
NTSTATUS HwBpEngineAdd(
    ...
    if ( ! HwBpEngine && ! ((INSTANCE *)Instance)->HwBpEngine ) {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if ( ! HwBpEngine ) {
        HwBpEngine = ((INSTANCE *)Instance)->HwBpEngine;
    }
...
NTSTATUS HwBpEngineRemove(
    ...
    if ( ! Engine && ! ((INSTANCE *)Instance)->HwBpEngine ) {
        return STATUS_INVALID_PARAMETER;
    }
...
NTSTATUS HwBpEngineDestroy(
    ...
    ((INSTANCE *)Instance)->Win32.RtlRemoveVectoredExceptionHandler( HwBpEngine->Veh );
...
LONG ExceptionHandler(
    ...
    if ( Exception->ExceptionRecord->ExceptionCode == STATUS_SINGLE_STEP )
    {
        ...
        BpEntry = ((INSTANCE *)Instance)->HwBpEngine->Breakpoints;
